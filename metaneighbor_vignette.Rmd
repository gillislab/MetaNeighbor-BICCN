...
```{r}
# DO NOT RUN: Code used to generate pre-trained MetaNeighbor model
...
if (FALSE) {
    ...
    write.table(pretrained_model,
                "pretrained_data/pretrained_biccn_mouse_mop_subclasses.txt")
    ...
    write.table(pretrained_model,
                "pretrained_data/pretrained_biccn_mouse_mop_clusters.txt")
}
```
...
pretrained_biccn = as.matrix(read.table(
    "pretrained_data/pretrained_biccn_mouse_mop_clusters.txt", check.names = FALSE
))
```
...
There are also three parameters to specify:
